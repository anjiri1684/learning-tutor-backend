...
func AdminDeleteUser(c *fiber.Ctx) error {
	...
	err := database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if user.Role == "teacher" {
			...
			if err := tx.Where("student_id = ?", userID).Delete(&models.Review{}).Error; err != nil { return err }
			if err := tx.Where("student_id = ?", userID).Delete(&models.Booking{}).Error; err != nil { return err }
			if err := tx.Where("student_id = ?", userID).Delete(&models.Waitlist{}).Error; err != nil { return err }
			if err := tx.Where("student_id = ?", userID).Delete(&models.LibraryResourceAccess{}).Error; err != nil { return err }
		}

		if err := tx.Where("sender_id = ?", userID).Delete(&models.Message{}).Error; err != nil { return err }
		if err := tx.Exec("DELETE FROM conversation_participants WHERE user_id = ?", userID).Error; err != nil { return err }
		if err := tx.Where("user_id = ?", userID).Delete(&models.Notification{}).Error; err != nil { return err }
		if err := tx.Exec("DELETE FROM library_resource_accesses WHERE library_resource_id IN (SELECT id FROM library_resources WHERE uploaded_by = ?)", userID).Error; err != nil { return err }
		if err := tx.Where("uploaded_by = ?", userID).Delete(&models.LibraryResource{}).Error; err != nil { return err }
